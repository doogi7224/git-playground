...
1. **적/투사체는 절대 개별 Node로 만들지 않는다.** `EnemyManager`가 `PackedFloat32Array` 배열로 관리하고 `MultiMeshInstance2D`로 렌더한다.
...
- 적 상태 = 배열(SoA). 렌더 = 타입별 MultiMesh 1개. 애니메이션은 셰이더에서 UV 오프셋 + 시간 기반 bob/squash로 GPU 처리.
...
**현재 상태: 프롬프트 0 (프로젝트 초기화) 완료. 다음은 프롬프트 1 = M0 프로토타입.**
