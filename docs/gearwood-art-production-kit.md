...
> 현재 에셋 방향은 밝은 페인터리 숲 모험 스타일이다. 기계/자연 상태 전환 관련 프롬프트와 규칙은 더 이상 사용하지 않는다. 현재 기준은 프로젝트 루트의 `CLAUDE.md`를 따른다.
...
> **대체됨(충돌 시 기본값 규칙 적용)**: 이 문서는 초기 컨셉아트/프롬프트 브리프(v0.1)로 "페인터리 픽셀아트" 스타일을 전제로 하지만, 더 최근에 추가된 `CLAUDE.md` 19장(비주얼/UI 리디자인 지침)이 전체를 픽셀아트로 강제 전환하지 않는다고 명시적으로 대체한다. 실제 구현은 절차적 드로잉 + AI 생성 스프라이트 혼합 방식이며, 픽셀아트 여부는 이 문서가 아니라 `CLAUDE.md` 19장을 따른다. 팔레트·라이팅·대비 규칙(Part B)은 여전히 참고 가치가 있다.
