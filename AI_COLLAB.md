...
- 현재: 사용자 실기기 피드백(심한 렉, 부자연스러운 움직임, 몬스터 외형 불연속)을 반영했다. 화면 밖 월드 오브젝트 컬링·고정 60Hz 물리 스텝·비가시 대형 배경 제거, 주인공/Brambleling/Chestnut Roller 4프레임 보행, 동일 외형 Acorn Hopper 상태 포즈, Bio-Coil 왜곡 축소를 적용했다. 넓은 가로 화면에서 16:9 고정 프레임이 만들던 좌우 검은 띠도 제거해 월드 배경이 전체 화면을 채운다. Root Cache는 작고 고정된 `?` 상자로 주 동선 3곳에 놓이며, 아래에서 점프 타격 시 자석/보호막/화살 +1/생명 +1 중 하나를 무작위 지급한다. 획득한 자석·보호막은 좌측 상단 전용 아이콘으로 표시하고, 자석은 종료 5초 전에 깜빡인다. Spring은 제거하고 기본 점프를 기존 Spring 강화 수준으로 높였다. 기존 Relic Pod는 활로 맞혀 여는 숲 과녁(Relic Target) 이미지로 교체했다. 밤송이 적은 평소 순찰하다가 같은 높이의 플레이어를 감지했을 때만 바라보는 쪽으로 굴러가며, 시작 위치를 순찰 구간 중앙으로 옮기고 첫 구간을 넓혀 경계 즉시 정지가 나지 않게 했다. Acorn Hopper는 제자리 복귀 대신 전방 도약·발판 끝 반전으로 움직인다. Overdrive/FLOW 시스템과 HUD는 제거하고, Steam Blower는 레벨·물리·렌더링·전용 에셋까지 완전히 제거했다. Root-Hook Cog와 Steam Boost도 제거했으며 Mirror는 1회 보호막으로 재설계했다.
